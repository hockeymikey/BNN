...
from cs231n.layer_utils_gpu import *
...
def average_pooling_forward(x):
  out = cp.mean(x, axis=(2, 3)) # cp.mean with (H, W) axis
  cache = (x)
  return out, cache
...
class ConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=(32), filter_size=7,
    ...
    ############################################################################
    # first conv layer
    W1 = cp.random.randn(num_filters[0], input_dim[0], filter_size, filter_size) / cp.sqrt(filter_size**2*input_dim[0] / 2)
    b1 = cp.zeros(num_filters[0])
    
    # second conv layer
    W2 = cp.random.randn(num_filters[1], num_filters[0], filter_size, filter_size) / cp.sqrt(filter_size**2*num_filters[0] / 2)
    b2 = cp.zeros(num_filters[1])
    
    # third conv layer
    W3 = cp.random.randn(num_filters[2], num_filters[1], filter_size, filter_size) / cp.sqrt(filter_size**2*num_filters[1] / 2)
    b3 = cp.zeros(num_filters[2])
    
    # fourth conv layer
    W4 = cp.random.randn(num_filters[3], num_filters[2], filter_size, filter_size) / cp.sqrt(filter_size**2*num_filters[2] / 2)
    b4 = cp.zeros(num_filters[3])
    ...
    # fifth fc layer
    W5 = cp.random.randn(int(num_filters[3]*H*W), hidden_dim) / cp.sqrt(num_filters[3]*H*W / 2.0)
    b5 = cp.zeros(hidden_dim)
    
    # fc-10 layer
    W6 = cp.random.randn(hidden_dim, num_classes) / cp.sqrt(hidden_dim / 2)
    b6 = cp.zeros(num_classes)
    ...
    self.params["gamma1"] = cp.full(num_filters[0], 1, dtype=cp.float32)
    self.params["beta1"] = cp.zeros(num_filters[0])
    self.params["gamma2"] = cp.full(num_filters[1], 1, dtype=cp.float32)
    self.params["beta2"] = cp.zeros(num_filters[1])
    self.params["gamma3"] = cp.full(num_filters[2], 1, dtype=cp.float32)
    self.params["beta3"] = cp.zeros(num_filters[2])
    self.params["gamma4"] = cp.full(num_filters[3], 1, dtype=cp.float32)
    self.params["beta4"] = cp.zeros(num_filters[3])
    self.params["gamma5"] = cp.full(hidden_dim, 1, dtype=cp.float32)
    self.params["beta5"] = cp.zeros(hidden_dim)
...
#    ff, _, HHf, WWf = W1.shape
    ...
    conv2, conv2_cache = conv_bn_relu_forward(conv1, W2, b2, gamma2, beta2, conv_param, bn_params[1])
    conv3, conv3_cache = conv_bn_relu_forward(conv2, W3, b3, gamma3, beta3, conv_param, bn_params[2])
    conv4, conv4_cache = conv_bn_relu_pool_forward(conv3, W4, b4, gamma4, beta4, conv_param, pool_param, bn_params[3])
    ...
    dx4, dw4, db4, dgamma4, dbeta4 = conv_bn_relu_pool_backward(dx5, conv4_cache)
    dx3, dw3, db3, dgamma3, dbeta3 = conv_bn_relu_backward(dx4, conv3_cache)
    dx2, dw2, db2, dgamma2, dbeta2 = conv_bn_relu_backward(dx3, conv2_cache)
    ...
    loss += 0.5 * self.reg*(cp.sum(W1**2)+cp.sum(W2**2)+cp.sum(W3**2)+cp.sum(W4**2)+cp.sum(W5**2)+cp.sum(W6**2))
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    end = time.time()
